...
Experiment with the table.



##### Discussion Ideas  

Use the sorting arrows to sort by player first then year. Subset the data to only show Derek Jeter, note use the search window.
...
Remove year to get the batting averages for each player. Experiment with the pulldown menus to create other summaries.  Sort using the small areas next to the aggregation pulldown menu. Switch year and player and determine the output.

#### Interactive Vector Plots  

**shiny**[@R-shiny]

Without developing our own custom visualization, which we could have done in [geogebra](https://www.geogebra.org/) , we decided to find an existing application.  There is an interactive visualization application in the geogebra library, go to https://www.geogebra.org/m/YrPAV6KK.  The example uses shooting baskets but gives the same idea as the reversal we saw in our problem.  We selected this interaction becomes it helps explain why the reversal occurs and the conditions under which it occurs.  
...
The fourth phase of the analysis is the creation of a knowledge product. In this case the reader(s) will be varied.

### Discussion Ideas  

What would be the best way to present the final conclusion that Derek Jeter is a better batter than David Justice?
...
http://floda31.com/2011/05/if-the-world-were-100-people/ 
https://www.r-bloggers.com/infographic-style-charts-using-the-r-waffle-package/ 
