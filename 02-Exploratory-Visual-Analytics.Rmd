...
In the next phase of our exploratory data analysis we will be visualizing the data. In Section \@ref(Viz1), we discussed some of the ideas of visualization. For the first part, we will be generating visualization to help understand the content of the data.

### First Visualizations

In our first visualization we need to try to find key varialbes from the 9 total variables. The first three variables, country, continent, and year are different from the last seven and will have to be dealt with in a different manner.
...
To get an overview of the data, let's plot the pairwise scatterplot for each of the continuous variables.

```{r fig-mat, fig.cap="Pairwise Scatterplots of Variables",warning=FALSE}
ggpairs(gapminder_va %>% filter(year==1950) %>% select(c(-1,-3)),aes(color=continent))
```
