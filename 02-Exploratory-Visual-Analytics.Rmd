...
An alternative way to view the data is with a dotplot and boxplot. We will start with a dotplot where the country is on the vertical axis and life expectancy is on the horizontal axis. Figure \@ref(fig:fig-dp1) gives the result. The vertical axis is too crowded and not
...
ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950,continent=="Asia"),aes(x=life_exp,y=reorder(country,life_exp),color=factor(year),size=density)) +
  geom_point() +
  scale_size(name="Density",
             breaks=c(10,100,1000,4000,6000),range=c(1,4)) +
  scale_color_discrete(name="Year") +
...
```{r fig-dp1, fig.cap="Side by Side Scatterplots of Fertility and Percentage of Population above 60 for 1950, 1982, and 2015 with Population Density",warning=FALSE,message=FALSE}
ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950),aes(x=life_exp,y=reorder(country,life_exp),color=factor(year),size=density)) +
  ...
  labs(x="Lief Expectancy",y="") +
  theme(axis.text.y=element_text(hjust = grid::unit(c(-2, 0, 2), "points")))
```


```{r}
ggplot(gapminder_va %>% 
         filter(year==1950 | year == 2015),aes(y=life_exp,x=reorder(continent,life_exp))) + 
geom_boxplot() + 
coord_flip() +
facet_grid(~year) +
theme_bw() +
...
The time series plots have the advantage of showing the entire time history instead of a few selected time periods. However, we are limited in how to compare the different variables simultaneouly. We could map variation as the line width to help understand the consistency within a continent. See Figure \@ref(fig:fig-ts4) for an example. There is not much difference in variance except for Europe where the variance is smaller and for the last 25 years is minimal.
...
gapminder_va_log %>% 
  ...
  ggplot(aes(x=year,y=Fertility,color=continent,group=continent)) + geom_line(aes(group=continent,size=Variance)) + 
...
ggpairs(aes(color=continent,alpha=.7,text=country),upper = list(continuous = "blank", combo = "box_no_facet", discrete =
