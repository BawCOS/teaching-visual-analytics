...
Finally, we will wrangle the data into the final form needed. The term used in tidy data; each row is an observation and each column a variable. As read in, the data is in a wide format where the value of years, which is a single variable, are used as column titles. We will switch to a long format.
...
Before proceeding, we will check the condition of the data and clean up any obvious issues. First compare the number of countries in our data with those in the **gapminder** package.
...
There are 142 countries in the **gapminder** data but only 139 in the data we collected that match those 142 countries. There might some names in **gapminder** that are different from the imported data. We will check this idea first.
...
To conclude, we clean up the workspace by removing temporary objects.
...
We are missing Taiwan and Reunion from the data and for the other three mismatches we have Republic names as Rep.
...
Next we bring in life expectancy data and since the source is the same as population we will have the same issue with the names of the Koreas and Yemen. We will be performing the same steps as section \@ref(Pop2), so will skip the narrative on many of the steps.
...
Next fertility rates of a country may be an indication of its developmental state. Let's get the data.
...
Before we interpolate, let's plot two countries to get an idea of the data.
...
Finally, we clean up the figure of our data by removing the background shading.
...
To compare two years, let's focus first on just two variables, fertility and life expectancy. If we want to minimize ink, we can use a theme based on Tufte's work.
...
Another approach to compare continents is to generate boxplots for each continent for a particular year. Figure \@ref(fig:fig-bp1) is an example. This makes it easy to compare the continents withing a year but more difficult to compare years.
