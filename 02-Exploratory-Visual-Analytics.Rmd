...
> "Visualization can surprise but doesn't scale well. Modeling scales well but cannot 
> surprise you." -- Hadley Wickham

> "We have the duty of formulating, of summarizing, and of communicating our conclusions,
> in intelligible form, in recognition of the right of other free minds to utilize 
> them in making their own decisions" -- Ronald Fisher  
...
This is an economic criteria.  Other criteria can be found on [Wikipedia](https://en.wikipedia.org/wiki/Developed_country).
...
First data is downloaded in a Excel spreadsheet from the [Gapminder](https://www.gapminder.org/data/) website.  We will use the package **readxl** to read it into `R`.  In addition, `R` has a package called **gapminder** which is a subset of the data from the Gapminder website. We will use it to get country names to reduce the size of our data and exclude countries with a large amount of missing data.  
...
Look at the first few rows of the data to get a feel for the data.  
...
Finally, wrangle the data into the final form needed. The term used is tidy data; each row is an observation and each column a variable. As currently imported, the data is in a wide format where the value of years, which is a single variable, are used as column titles. Switch to a long format.
...
There are 142 countries in the **gapminder** data but only 139 in the data we collected that match those 142 countries. There might some names in **gapminder** that are different from the imported data. Check this idea first.
...
Finally get rid of the countries not in the **gapminder** package.
...
Next look for missing observations.
...
Add the continent names using a `join` function.  
...
We are missing Taiwan and Reunion from the data and for the other three mismatches we have Republic named as Rep.
...
Now merge the data with the population data and fill in the missing values.
...
Fill in data for Taiwan and Reunion. Taiwan is missing its density. The area of Taiwan is [36,000 squared kilometers](https://www.statista.com/statistics/321367/taiwan-land-area/) and the area of Reunion is 2510, .
...
Fill in the years prior to 1961 with the values from 1961.  
...
Fill in the missing value in the tail with the most recent values.
...
Calculate density by dividing population by area.
...
Next bring in life expectancy data. Since the data source is the same as population we will have the same issue with the names of the Koreas and Yemen. We will be performing the same steps as section \@ref(Pop2), so the narrative on many of the steps is skipped.
...
Taiwan again has missing values. Estimate the fertility with average of the previous 5 years.
...
This data came from a different source, so be careful about the country names.

```{r}
countries[!countries %in% unique(gapminder_above60$country)]
```

Taiwan is still a problem but now there are three other countries that are in the **gapminder** package and not in the new data set. This is a problem with naming similar to what we found when finding density. Correct those first three countries.
...
Remove Taiwan since we have no data on it. While at it, remove Hong Kong, Puerto Rico, and Reunion. We could drop all data prior to 1980 but this is 30 years of data. Instead, we chose to drop these four small countries.
...
First remove the projections.

```{r}
gapminder_above60 <- gapminder_above60 %>% filter(year <= 2015)
```

Before interpolating, plot two countries to get an idea of the data.
...
Now use a spline. This process fits a cubic spline between each point and will be smoother than the linear interpolation.  
...
To avoid the temporary variables, use the code below in one step.
...
Interpolate for every country and create our final data frame. Use the map function from the **purrr** package.  
...
Finish by adding the continents.
...
The first few rows of the final data frame:
...
We have two problems left, the percentage of the population above 60 has less countries than the other data sets and life expectancy has one more year, 2016, than the other data sets. Resolve these problems by dropping the 4 countries and the year 2016.
...
Drop the countries by using the join function.
...
gapminder_va <-gapminder_above60 %>%
  ...
  mutate(country=factor(country),pop=round(pop/100000,2),
         ...
         mortality=round(mortality,2),density=round(density,2)) 
```  
...
There is a problem with the names of the countries again. 
...
Determine which countries are in the new data set and then correct the names.
...
Fix the names. We know *Korea, Rep.* is correct, let's remove it.
...
Now get the data from the **gapminder** package and compare the years to determine how to adjust.
...
To recap the data collection process, we will explain each variable. There are 138 countries on 5 continents for the years 1950 through and including 2015. Table \@ref(tab:final-tab) gives the first six rows of the data.
...
## Visualization of Data {#Viz2}

In the next phase of our exploratory data analysis we will be visualizing the gapminder data. In Section \@ref(Viz1), we discussed some of the ideas of visualization. As a first step, we will be generating a visualization to help us understand the content of the data. This step is important in exploration as it often leads to insights into the next step of the analysis.

### Summary Visualizations

In the first visualization we attempt to understand the variables and their relationship to each other. There are nine total variables. The first three variables, country, continent, and year are different from the last six and will have to be dealt with in a different manner. Country and continent are label variables. Year is a discrete count variable. The other six variables are continuous and can be plotted against each other on a scatterplot.
...
We are doing exploration. We need to focus on detection of interesting features. Our choice of plots and colors should be driven by this goal.

#### Summarize Data 

Complete a descriptive summary of the data.
...
To get an overview of the data, plot the pairwise scatterplots for each of the continuous variables. We still need to address the issue with year, but for now we will just pick the year 1950.
...
Put population and density on a logarithm scale.
...
```{r tab-va-log,echo=FALSE,warning=FALSE}
gapminder_va_log %>% 
  head() %>% 
  knitr::kable(caption="Data with Population and Density on a Log Scale",digits=2,format='html') %>%
  kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12)
```

Next plot this new data set and add continent as a color.
...
Combine Oceania into Europe; also check that our work is correct.
...
Now recreate the plot and add a legend and transparency to aid in visualization.

```{r fig-mat3, fig.cap="Pairwise Scatterplots of Variables with Oceania Removed",warning=FALSE,echo=FALSE,message=FALSE}
ggpairs(gapminder_va_log %>% filter(year==1950) %>% select(c(-1, -3)),aes(color=continent,alpha=.7),legend=c(1,1))
```  
...
### Adding Time to Plots  

To understand the progression of countries over time, look at different years. In the previous section we looked only at the year 1950. In this section we will address different years.

#### Adding the Year 2015  

First look at the scatterplot matrix for the final year, 2015.


```{r fig-mat4, fig.cap="Pairwise Scatterplots of Variables for 2015",warning=FALSE,echo=FALSE,message=FALSE}
ggpairs(gapminder_va_log %>% filter(year==2015) %>% select(c(-1, -3)),aes(color=continent,alpha=.7),legend=c(1,1))
```  
...
To compare two years, focus first on just two variables, fertility and life expectancy. If we want to minimize ink, we can use a theme based on [Tufte's work](https://cran.r-project.org/web/packages/ggthemes/vignettes/ggthemes.html).
...
ggplot(gapminder_va_log %>% filter(year==2015 | year==1950),aes(x=life_exp,y=fertility,color=continent)) +
  ...
  facet_wrap(~year) +
  theme_tufte() +
  labs(x="Life Expectancy",y="Fertility",color="Continent")
```
...
We can account for population by assigning it to another aesthetic such as size. This will work better if we go back to original data. Using population density will also be better than population because this variable accounts for the size of the country. 

First, combine Oceania with Europe.
...
ggplot(gapminder_va %>% filter(year==2015 | year==1950),aes(x=life_exp,y=fertility,color=continent,size=density)) +
  ...
  scale_size(name="Density",
             breaks=c(10,100,1000,4000,6000),range=c(1,7)) +
  scale_colour_brewer(palette="Dark2",name="Continent") +
  facet_wrap(~year) +
  theme_tufte() +
  labs(x="Life Expectancy",y="Fertility")
```

We are getting tired of adding the theme element so we will just set it as the default.
...
Using Figure \@ref(fig:fig-sbs3) answer the following questions:
...
We still have not adequately incorporated year into the visual exploration. As a next step, add a year that is between the end points, in this case 1982.
...
The shift from 1950 to 1982 was mostly an increase in life expectancy with a moderate decrease in fertility. although for some of the African countries, the fertility increased.  The period from 1982 to 2015 saw a big decrease in fertility with a minor increase in life expectancy.

We could incorporate year by have a slider function to select year or another option is to animate the plot by year.
...
From Figure \@ref(fig:fig-sbs5) answer the following questions:
...
We need the capability to brush and identify. In 2015 one Asian country has high percentage of population above 60.

### Box and Dot Plots  

An alternative way to view the data is with a dotplot and boxplot. Start with a dotplot where the country is on the vertical axis and life expectancy is on the horizontal axis. Figure \@ref(fig:fig-dp1) gives the result. The vertical axis is too crowded and not readable even though we reduced the default font size. Notice also how the countries within continents are arranged in decreasing life expectancy.

```{r fig-dp1, fig.cap="Plots of Life Expectancy for Each Country grouped by Continent for 1950, 1982, and 2015 with Population Density",warning=FALSE,message=FALSE}
ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950),aes(x=life_exp,y=reorder(country,life_exp),color=factor(year),size=density)) +
  geom_point() +
  scale_size(name="Density",
             breaks=c(10,100,1000,4000,6000),range=c(1,4)) +
  scale_color_discrete(name="Year") +
  facet_grid(continent~.,scale= 'free_y', space = 'free_y') +
  labs(x="Life Expectancy",y="") +
  theme_tufte(base_size = 8)
```

We could break each plot down into an individual continent. For example Figure \@ref(fig:fig-dp2) shows the plot for Europe. This plot is much easier to read but we lose the ability to compare the other three continents.

```{r fig-dp2,fig.cap="Plot of Life Expectancy for Europe for the Years 1950, 1982, and 2015"}
ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950,continent=="Europe"),aes(x=life_exp,y=reorder(country,life_exp),color=factor(year),size=density)) +
  geom_point() +
  scale_size(name="Density",
             breaks=c(10,100,1000,4000,6000),range=c(1,4)) +
  scale_color_discrete(name="Year") +
...
A third attempt is to make a two dimensional matrix plot of the four individual plots. Figure \@ref(fig:fig-dp3) shows the results. Africa is still a bit crowded but individual country names are visible.

```{r fig-dp3,fig.cap="Matrix Plot of Life Expectancy for the Years 1950, 1982, and 2015"}
ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950),
    aes(x=life_exp,y=reorder(country,life_exp),color=factor(year),size=density)) +
     geom_point() +
     scale_size(name="Density",
                breaks=c(10,100,1000,4000,6000),range=c(1,4)) +
     scale_color_discrete(name="Year") +
...
Another approach to compare continents is to generate boxplots for each continent for a particular year. Figure \@ref(fig:fig-bp1) is an example. This makes it easy to compare the continents withing a year but more difficult to compare across years.

```{r fig-bp1,fig.cap="Boxplots of Life Expectancy for Each Continent for the Years 1950 and 2015"}
ggplot(gapminder_va %>% 
         filter(year==1950 | year == 2015),
         aes(y=life_exp,x=reorder(continent,life_exp))) + 
geom_boxplot() + 
coord_flip() +
facet_grid(~year) +
...
What are the advantages of the boxplots versus the dotplots?  
What trends do we see in the boxplots?  
How can we deal with the variable year in a better manner?  
What are some ways to address the issue of the crowded vertical axes in the dotplots?  

In the boxplot we get an idea of the variability of life expectancy within each continent. It is easy to compare continents as well. However, we don't have the ability to analyze on the country level. 

In general, we see that life expectancy has improved for all continents. Europe still has the best results but the difference between continents in 2015 is much smaller than it was in 1950. The variability is also decreasing as the width of the boxplots is getting smaller.  

The side by side boxplots has done a good job of accounting for years but we may need to use an animation to explore all years.  

The crowded axes is difficult. We may want to sample countries from each quartile to reduce the number of countries on each axes. Another approach is to offset the labels slightly but this has a small chance of making a difference.  

### Parallel Coordinates Plot

Another type of plot for our data is a parallel coordinates plot. This plot scales the data first so that the size of the variables will not dominate the plot. We will also use the data with density and population on a logarithmic scale. The order of the variables is key as well. 
...
We put fertility and mortality first since Europe was lower on both of the metrics in 1950 than the other continents. We place population and population density at the end since there was no discernible pattern. 
...
```{r fig-pc2, fig.cap="Parallel Coordinate Plot for 1950 and 2015. Population and Density are removed."}
ggparcoord(gapminder_va_log %>% filter(year==1950 | 
...
Next we will plot year on the horizontal axis and one of the other variables of interest on the vertical axis. Figure \@ref(fig:fig-ts1) is an example. This has a different line for each country and as a result contains too much information. To help with this problem, could just summarize each continent instead.

```{r fig-ts1,fig.cap="Fertility versus Year"}
gapminder_va_log %>% ggplot(aes(x=year,y=fertility,group=country,color=continent)) + 
  geom_line(alpha=0.7)
```

In Figure \@ref(fig:fig-ts2) we have simplified the plot by finding the median for each continent for each year. This of course assumes that the countries within a continent are similar. This is not a particularly good assumption for the Americas.

```{r fig-ts2,fig.cap="Median Fertility versus Year for each Continent"}
gapminder_va_log %>% 
  group_by(continent,year) %>% 
  summarise(Fertility=median(fertility)) %>%
  ggplot(aes(x=year,y=Fertility,color=continent)) + geom_line() +
  labs(x="Year",y="Median Fertility")
```

In Figure \@ref(fig:fig-ts3) we have a nice plot of the two variables fertility and life expectancy plotted next to each other. We again summarized the data at the continent level by taking the median. This erases individual country level data but we can see overall trends for each continent.

```{r fig-ts3,fig.cap="Median Fertility and Median Life Expectancy versus Year for each Continent"}
gapminder_va_log %>% 
  group_by(continent,year) %>% 
  summarise(Fertility=median(fertility),"Life Expectancy"=median(life_exp)) %>%
  gather(event,median,Fertility,"Life Expectancy") %>% 
  ggplot(aes(x=year,y=median,color=continent)) + geom_line() +
  labs(x="Year",y="") +
...
What are the advantages and disadvantages of each of the plots?  
How can we account for country to country variation within a continent?  
What are some unusual insights?

The time series plots have the advantage of showing the entire time history instead of a few selected time periods. However, we are limited in how to compare the different variables simultaneously. We could map variation as the line width to help understand the consistency within a continent. See Figure \@ref(fig:fig-ts4) for an example. There is not much difference in variance except for Europe where the variance is smaller and for the last 25 years is minimal.

Around 2000, Africa had a large drop in life expectancy. In the late 1950s, Asia had a big drop in fertility. Conjectures about the cause can be made and researched. These trends would be hard to see in previous plots.


```{r fig-ts4,fig.cap="Median Fertility with Standard Deviation as line width for each Continent"}
gapminder_va_log %>% 
  group_by(continent,year) %>% 
  summarise(Fertility=median(fertility),Variance=sd(fertility)) %>%
  ggplot(aes(x=year,y=Fertility,color=continent,group=continent)) + 
  geom_line(aes(group=continent,size=Variance)) + 
...
Another approach to summarizing the data is to use [geofaceting](http://ryanhafen.com/blog/geofacet). The plots of interest are put in geographical relationship to each other. Figure \@ref(fig:fig-ts5) is an example. Here we look only at the European continent as an example, this is because the European grid map already exists. For each country we have a plot of life expectancy versus time. We show the code below but the package **Bookdown** was not able to render the graphic so we had export the image from a local instance of `R` and then import into this document.

```
# Redefine the grid to include the countries we have in our data
my_eu_grid<-eu_grid1[c(-6,-13,-14,-15,-16,-18,-25),]
```


```
ggplot(gapminder_va_log, aes(year, life_exp)) +
    geom_line(color="steelblue") +
    facet_geo(~ country, grid = my_eu_grid) +
    scale_x_continuous(labels = function(x) paste0("'", substr(x, 3, 4))) +
    ylab("Life Expectancy") +
    theme_bw()
```

```{r fig-ts5,cap="Time Series of Life Expectancy for each European Country Plotted as Facets based on Geography",echo=FALSE}
knitr::include_graphics("./images/Geofact.png",dpi=90)
```

#### Discussion Ideas  

What are the advantages and disadvantages of each of this plot?  

This plot requires the reader to know geography. It is also difficult to compare countries that are separated by distance. However, it does exhibit geographical effects along with time. It would be difficult to build a plot for the entire world.

### Clustering  

We have been using visualization to find groupings of data. An alternative is to use hierarchical clustering. In this section we apply hierarchical clustering to our data.

```{r}
gap_clus <- hclust(dist(gapminder_va_log %>% filter(year == 1950) %>% 
              select(4:9)))

plot(gap_clus,labels=(gapminder_va_log %>% filter(year == 1950) %>% 
      select(continent)%>% mutate(continent=as.character(continent)))[[1]],
     xlab="Continent")
```

There appear to be five strong clusters. Separate out these clusters.

```{r echo=FALSE}
clust_names<-gapminder_va_log %>% filter(year == 1950) %>% select(1,2) %>% mutate(country=as.character(country),continent=as.character(continent))
```

Plot a table of continents and cluster groups.

```{r}
clus_res<-cutree(gap_clus,k=5)
table(clust_names[[2]],clus_res)
```

Table \@ref(tab:tab-clust1) are the countries in the second cluster.

```{r tab-clust1,echo=FALSE,warning=FALSE}
clust_names[which(clus_res==2),] %>% arrange(continent) %>% knitr::kable(caption="Second Grouping of Countries in 1950")
```

Table \@ref(tab:tab-clust2) are the countries in the first cluster.

```{r tab-clust2,echo=FALSE,warning=FALSE}
clust_names[which(clus_res==1),] %>% arrange(continent) %>% 
  knitr::kable(caption="First Grouping of Contries in 1950")
```

Next examine the year 2015. 

```{r}
gap_clus2 <- hclust(dist(gapminder_va_log %>% filter(year == 2015) %>% 
              select(4:9)))

plot(gap_clus2,labels=(gapminder_va_log %>% filter(year == 2015) %>% 
      select(continent)%>% mutate(continent=as.character(continent)))[[1]],
     xlab="Continent")
```

There are now 4 distinct clusters instead of 5.  

```{r}
clus_res2<-cutree(gap_clus2,k=4)
table(clust_names[[2]],clus_res2)
```

Asia, Europe, and the Americas are in a single group and Africa is split into three separate groups. Group 3 is interesting because it is only four countries. Table \@ref(tab:tab-clust3) contains the four countries.

```{r tab-clust3,echo=FALSE,warning=FALSE}
clust_names[which(clus_res2==3),] %>% arrange(continent) %>% 
  knitr::kable(caption="Thrid Grouping of Contries in 2015")
```

Note that this analysis used an Euclidean distance to determine similarity between countries. Since the variables were an roughly similar scales, we did not scale the data. Although mortality and life expectancy were the largest in size.  Another approach is to use correlation between countries to group.


```{r}
dd<-as.dist(1-cor(t(gapminder_va_log %>% filter(year == 1950) %>% 
      select(4:9))))


gap_clus3 <- hclust(dd)

plot(gap_clus3,labels=(gapminder_va_log %>% filter(year == 1950) %>% 
      select(continent)%>% mutate(continent=as.character(continent)))[[1]],
     xlab="Continent")
```

Now there are three or four strong groups. We will use three.

```{r}
clus_res3<-cutree(gap_clus3,k=3)
table(clust_names[[2]],clus_res3)
```

Table \@ref(tab:tab-clust4) is a listing on the countries grouped together in group 3. 

```{r tab-clust4,echo=FALSE,warning=FALSE}
clust_names[which(clus_res3==3),] %>% arrange(continent) %>% 
  knitr::kable(caption="Third Grouping of Contries in 2015")
```

Now repeat for 2015.

```{r}
dd<-as.dist(1-cor(t(gapminder_va_log %>% filter(year == 2015) %>% 
      select(4:9))))


gap_clus4 <- hclust(dd)

plot(gap_clus4,labels=(gapminder_va_log %>% filter(year == 2015) %>% 
      select(continent)%>% mutate(continent=as.character(continent)))[[1]],
     xlab="Continent")
```

Again, there are either 3 or 4 strong groups.  

```{r}
clus_res4<-cutree(gap_clus4,k=3)
table(clust_names[[2]],clus_res4)
```

We see that Africa is split into two groups and while Asia, Europe, and the Americas are grouped together. This is the trend we have been observing in our analysis. Table \@ref(tab:tab-clust5) is a listing on the countries grouped together in group 2. 

```{r tab-clust5,echo=FALSE,warning=FALSE}
clust_names[which(clus_res4==2),] %>% arrange(continent) %>% 
  knitr::kable(caption="Second Grouping of Contries in 2015")
```

#### Discussion Ideas  

What are the advantages and disadvantages of clustering and listing clusters?
What are some additional things you would suggest to do with clustering?  

The clustering does the grouping on all variables instead of just two as in previous plots. The difficulty is selecting the number of groups, the similarity measure, and the grouping metrics. The analysis above using a correlation metric appears to answer our original questions best

A good way to visualize the clusters is with a choropleth plot. 

#### Choropleth Plot  

The code for this section was adopted from this [website](https://rud.is/b/2015/07/09/faceted-world-population-by-income-choropleths-in-ggplot/)

The following code gets the data. We have already done this and saved it in the data folder associated with this project.
...
Load the library to read the json file.
...
world <- readOGR("data/world-geo.json", layer="world-geo")
...
Check that the countries have the same names as those in the *gapminder* package.

```{r}
clust_data<-cbind(clust_names,as.factor(clus_res3))
clust_data[!(clust_data[,1] %in% as.character((world_map %>% distinct(id))[,1])),1]
``` 

Save the data to a new object.  
```{r}
my_world_map <- world_map
world_map_names <- world_map %>% distinct(id)
world_map_names<-levels(world_map_names[,1])
```

Find the name in the world map data frame. As an example, we use Yemen.

```{r}
world_map_names[grep("Yemen",world_map_names)]
```

Fix all the names.

```{r}
my_world_map$id<-gsub("North Korea","Korea, Dem. Rep.",my_world_map$id)
my_world_map$id<-gsub("South Korea","Korea, Rep.",my_world_map$id)
my_world_map$id<-gsub("Yemen","Yemen, Rep.",my_world_map$id)
my_world_map$id<-gsub("Democratic Republic of the Congo","Congo, Dem. Rep.",my_world_map$id)
my_world_map$id<-gsub("Republic of the Congo","Congo, Rep.",my_world_map$id)
my_world_map$id<-gsub("Burma \\(Myanmar\\)","Myanmar",my_world_map$id)
my_world_map$id<-gsub("Slovakia","Slovak Republic",my_world_map$id)
my_world_map$id<-gsub("Ivory Coast","Cote d'Ivoire",my_world_map$id)
my_world_map$id<-gsub("Guinea Bissau","Guinea-Bissau",my_world_map$id)
```

Figure \@ref(fig:fig-choro1) is the plot for 1950.  

```{r fig-choro1, fig.cap="Choropleth for Clustering of Countries in 1950"}
gg <-ggplot(my_world_map,aes(map_id=id)) 
gg <- gg + geom_map(map=my_world_map,fill='white',color='black',size=0.15) +expand_limits(x=my_world_map$long,y=my_world_map$lat)

gg + geom_map(map=my_world_map,data=clust_data,aes(map_id=country,fill=as.factor(clus_res3)),color="black",size=0.15) + scale_fill_manual(values=c("#fff7bc", "#fec44f","#d95f0e"),name="Cluster") + theme(legend.title=element_blank())
```

And now for 2015, Figure \@ref(fig:fig-choro2) is the choropleth. Notice that the group numbers are arbitrary.

```{r echo=FALSE}
clust_data<-cbind(clust_names,as.factor(clus_res4))
```

```{r fig-choro2, fig.cap="Choropleth for Clustering of Countries in 2015"}
gg <-ggplot(my_world_map,aes(map_id=id)) 
gg <- gg + geom_map(map=my_world_map,fill='white',color='black',size=0.15) +expand_limits(x=my_world_map$long,y=my_world_map$lat)

gg + geom_map(map=my_world_map,data=clust_data,aes(map_id=country,fill=as.factor(clus_res4)),color="black",size=0.15) + scale_fill_manual(values=c("#fff7bc", "#fec44f","#d95f0e"),name="Cluster") + theme(legend.title=element_blank())
```
#### Discussion Questions  

1. What are the advantages and disadvantages of this plot?  
2. What features are apparent in these plots?  

The plot makes it is easy to see the clustering of countries. However, the reader must know geography as there are no popups with country information and no ability to zoom. There is no way to interact with the maps.  

It is clear that the countries are getting more similar in terms of the variables we have selected as time has progressed. However, Africa and parts on Latin America still lag.

### Conclusion from Visualization  

From these visualizations, we can see that most of the countries in the world have, over time, become similar in the metrics we selected with the exception of a large portion of Africa.  In general there is an increase in life expectancy while there is a decrease in fertility.  

At this point we have only done data visualization. We next add interactivity to the plots.

## Interactions with Visualizations  

In \@ref(Viz2) we explored static visualizations of our data, sometimes referred to as dataviz. This endeavor was fruitful is that we discovered some important insights. However, we did reach some limitations. Going back to \@ref(fig:fig0), the use of interaction in our visual exploration will help overcome some of these limitations. This section implements interaction as the next step in the visual exploration. We will make use of the `R` implementation of the plotly javascript library, **plotly** [@R-plotly].

### Interactive Data Summaries  

We will start our exploration with the plots from Section \@ref(Viz2) and data from Section \@ref(DataCol2). As a reminder, some of the shortcomings we found in our static visualizations were the inability to handle time, not being able to identify data points, and no ability to link data points together across different plots.

#### Final Data  

We presented our data in Table\@ref(tab:final-tab) but the table only contained the first several rows.  Figure \@ref(fig:tab-it1) is an interactive table, note **knitr** and **bookdown** require the use of the label `figure` instead of `table` in the title for items from the package **DT** [@R-DT]. In this interactive table, we can change the number of rows, sort, subset, and scroll.

```{r tab-it1,fig.cap="Interactive Table of Final Data",echo=FALSE}
DT::datatable(gapminder_va,rownames=FALSE,options=list(scrollX=TRUE))
```

#### Discussion Ideas  

What is the life expectancy of the United States in 1984?  
Which country in Europe had the highest mortality in 1984?  

### Interactive Scatterplot Matrix  

We started our visual exploration of our data with a matrix scatterplot. Let's use this same visualization, but now we will add the ability to interact with the plot. Since the plot will be too crowded with all the variables, we will drop population and density and we will remove the density plots on the diagonal and the correlation information in the upper portion of the plot. The density plots do not work well with the interaction software.

```{r fig-ip1,fig.cap="Interactive Scatterplot Matrix for 1950",warning=FALSE,message=FALSE}
gapminder_va_log %>% filter(year==1950) %>%
ggpairs(aes(color=continent,alpha=.7,text=country),
  upper = list(continuous = "blank", combo = "box_no_facet", discrete =
  "facetbar", na = "na"),diag = list(continuous =
  "blankDiag", discrete = "barDiag", na = "naDiag"),columns = 4:7 ) %>%
ggplotly(tooltip = c('continent','x','y','country'))
```

#### Discussion Questions  

Spend some time experimenting with Figure \@ref(fig:fig-ip1). The toolbar at the top of the plot has some options to zoom, pan, and screen capture. Also by placing the cursor over a data point, a popup window reveals information about that point.

What are the countries that have low fertility and low life expectancy?  
Using the pan and zoom features, find the the life expectancy for the country with the largest fraction of the population above the age of 60.  
What is the country in the previous question?  

### Interactive Parallel Coordianates Plot  

We also used parallel coordinates plots in our visual analysis. In Figure \@ref(fig:fig-ip2) we have added some interactivity to this plot.

```{r fig-ip2,fig.cap="Interactive Scatterplot Matrix for 1950",warning=FALSE,message=FALSE}
ggplotly(ggparcoord(gapminder_va_log %>% filter(year==1950 | 
   year==2015),columns=c(4:7),group='continent',
   order = c(6,7,5,4),alphaLines = 0.5,mapping = aes(text=paste("country:", country))) +
   scale_colour_brewer(palette="Dark2",name="Continent") +
   facet_wrap(~year),tooltip = c('continent','x','text'))
```  
...
subplot(
plot_ly(gap_1950,x=~life_exp,y=~fertility) %>% add_markers(color=~continent),
plot_ly(gap_2015,x=~life_exp,y=~fertility) %>% add_markers(color=~continent),shareY=TRUE) %>% hide_legend()

p1<-ggplot(gap_1950,aes(x=life_exp,y=fertility,color=continent))+geom_point()
p2<-ggplot(gap_2015,aes(x=life_exp,y=fertility,color=continent))+geom_point()
...
Spend some time experimenting with Figure \@ref(fig:fig-ip2). Select the continents in the legend to "turn on" and "turn off" the data.

What are some advantages and disadvantages of this plot?
What features should be added and which are too much?

The plot are not linked. When we select a country in the right panel it is not highlighted in the left panel. 

One of the advantages of this plot is we have hover information although due to a limitation in the software we don't get the country name. We can also remove the lines by select the legend names.

The zooming capability is not much use in this plot and it would be a benefit to interactively reorganize the variables on the x-axis.
...
```{r}
gap_1950 <- SharedData$new(gapminder_va_log %>% filter(year==1950),group="gap_subset")
gap_2015 <- SharedData$new(gapminder_va_log %>% filter(year==2015),group="gap_subset")

bscols(widths=c(1,5,5),filter_checkbox("continent", "Continent", gap_1950, ~continent),
d3scatter(gap_1950, ~life_exp, ~fertility, ~continent,width = "100%"),
d3scatter(gap_2015, ~life_exp, ~fertility, ~continent,width = "100%")
)
``` 


Use rbokeh rid_plot https://bokeh.pydata.org/en/latest/docs/user_guide/interaction/linking.html
```{r}
...
```{r}
sd <- SharedData$new(gapminder_va_log %>% filter(year==1950 |
      year==2015) %>% mutate(country=as.character(country)), ~country)

p <- plot_ly(sd)

ggplotly(p,tooltip=c("x","country"))
```
...
p <- ggplot(sd, aes(life_exp, fertility,color=continent,text=country,group=year)) +
  geom_point() +
  facet_wrap(~ year)
...
ggplotly(ggplot(gapminder_va %>% filter(year==1982 |year==2015 | year==1950),aes(x=life_exp,y=country,color=factor(year),size=density)) +
  geom_point() +
  scale_size(name="Density",
             breaks=c(10,100,1000,4000,6000),range=c(1,7)) +
...
subplot(ggplotly(ggplot(gapminder_va_log %>% filter(year==1950),aes(y=above_60,x=fertility,color=continent,text = paste("country:", country))) +
geom_point(alpha=.7),tooltip="text"),ggplotly(ggplot(gapminder_va_log %>% filter(year==2015),aes(y=above_60,x=fertility,color=continent,text = paste("country:", country))) +
geom_point(alpha=.7),tooltip=c('continent','x','y','text')))
...
for(ii in seq_len(nrow(nms))) {
     splom_list[[ii]] <- figure(width = 200, height = 200, tools = tools,
         ...
         ly_points(nms$Var1[ii], nms$Var2[ii], data = iris,
                   color = Species, size = 5, legend = FALSE,hover = c(nms$Var1[ii], nms$Var2[ii]))
 }
...
#### Animations 
From https://plotly-book.cpsievert.me/key-frame-animations.html 
...
Choropleth map in leaflet
https://www.r-graph-gallery.com/183-choropleth-map-with-leaflet/ 
http://www.r-graph-gallery.com/168-load-a-shape-file-into-r/
...
https://github.com/simonepri/geo-maps/blob/master/info/countries-land.md#countries-land
world2 <- geojsonio::geojson_read("data/countries-land-10km.geo.json", what = "sp")
> class(world2)

#Leaflet on my world sp dataframe from above
...
m %>% addPolygons()
names(world)
https://rstudio.github.io/leaflet/choropleths.html Is code to help with map