...
We presented our data in Table\@ref(tab:final-tab) but the table only presented the first several rows.  Figure \@ref(fig:tab-it1) is an interactive table, note **knitr** and **bookdown** require the title of figure for items from the package **DT**. We can change the number of rows and scroll to see the entire table.
...
ggplotly(ggparcoord(gapminder_va_log %>% filter(year==1950 | 
   year==2015),columns=4:9,group='continent',
   order = c(6,7,5,4,8,9),alphaLines = 0.5,mapping = aes(text=country)) +
   scale_colour_brewer(palette="Dark2",name="Continent") +
   facet_wrap(~year))
```  
